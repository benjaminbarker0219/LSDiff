...
class GaussianDiffusion:
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            model_output, cal,gra = model(x_t, self._scale_timesteps(t), **model_kwargs)
            ...
            terms["loss_cal"] = mean_flat((res - gra) ** 2)
...
# import string
# import random
# from .ls12 import compute_curvature
# from .lse import levelset_evolution, gradient_sobel
# def standardize(img):
#     mean = th.mean(img)
#     std = th.std(img)
#     img = (img - mean) / std
...
#     ####
#     def p_mean_variance(
#         self, model, x_ori, t,phi,i,clip_denoised=True, denoised_fn=None, model_kwargs=None
#     ):
#         if model_kwargs is None:
#             model_kwargs = {}
#         B, C = x_ori.shape[:2]
...
#         assert t.shape == (B,)
#         model_output = model(x_ori, self._scale_timesteps(t), **model_kwargs)
#         if isinstance(model_output, tuple):
#             model_output, cal = model_output
          
#         x=x_ori[:,-1:,...]  #loss is only calculated on the last channel, not on the input brain MR image
#         phi_0 = cal +x 
#         def rgb_to_gray(img):
#             # 计算灰度值：0.299 R + 0.587 G + 0.114 B
#             weights = torch.tensor([0.299, 0.587, 0.114], device=img.device).view(1, 3, 1, 1)
#             gray = (img * weights).sum(dim=1, keepdim=True)  # 变成 (B, 1, H, W)
#             return gray
#         # if i == 999:
#         #     phi_0 = x_ori[:, :3, :, :]
#         #     phi_0 = rgb_to_gray(phi_0)
#         #     phi_0 = phi_0 + x
...
#         def _compute_score_gradient(x_t,t,model_output):#噪声图像的梯度
#             return (_extract_into_tensor(1.0/(self.sqrt_alphas_cumprod+1.0),t,x_t.shape)*(x_t-model_output))
    

#         def _compute_v(score_gradient, epsilon=1e-5):
        
#             norm = torch.norm(score_gradient, dim=-1, keepdim=True) + epsilon
#             v = score_gradient / norm
#             return v
...
#                     self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output)
#                 )
#                     #x_1= pred_xstart#torch.cat((phi, x), dim=1) 
#                     pt = _compute_score_gradient(x,t,model_output)
#                     v = _compute_v(pt)
#                     #vf = gradient_sobel(x, split=False)
#                     phi  = levelset_evolution(phi=phi_0 ,pt=pt,v=v)
                    
#                     #phi,_ = torch.split(phi,1,dim=1)
#                 #     phi = process_xstart(
#                 #     self._predict_xstart_from_eps(x_t=phi , t=t, eps=model_output)
#                 # )
#                 elif i==0:
...
#                     v = _compute_v(pt)
#                     #vf = gradient_sobel(x, split=False)
#                     phi  = levelset_evolution(phi=phi ,pt=pt,v=v)
#                     #phi,_ = torch.split(phi,1,dim=1)
#                 #     phi = process_xstart(
#                 #     self._predict_xstart_from_eps(x_t=phi , t=t, eps=model_output)
#                 # )
...
#         out = self.p_mean_variance(
# #这个out里面有  "mean": model_mean, "variance": model_variance,
# # #"log_variance": model_log_variance, "pred_xstart": pred_xstart,  'cal': cal, 'phi':phi
#             model,
#             x,
#             t,
#             phi,
...
#         return {"sample": sample, "pred_xstart": out["pred_xstart"], "cal": out["cal"],"phi":out["phi"]}
...
#         return final["sample"], x_noisy, img, final["cal"], cal_out,final["phi"]
...
#         assert isinstance(shape, (tuple, list))
#         if noise is not None:
#             img = noise#x_noisy加噪声的原图  
#         else:
#             img = th.randn(*shape, device=device)
#         indices = list(range(time))[::-1]#生成一个从大到小的列表（54321）
#         org_c = img.size(1)
#         org_MRI = img[:, :-1, ...]      #original brain MR image
#         if progress:
#             # Lazy import so that we don't depend on tqdm.
