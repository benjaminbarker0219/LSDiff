...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        if high_way:
            features = 32
            self.hwm1 = Generic_UNet(self.in_channels - 1, features, 1, 5, anchor_out=True, upscale_logits=True)
            self.hwm2 = Generic_UNet(self.in_channels - 1, features, 1, 5, anchor_out=True, upscale_logits=True)
    ...
    def highway_forward1(self,x, hs = None):
        return self.hwm1(x,hs = None)
    def highway_forward2(self,x, hs = None):
        return self.hwm2(x,hs = None)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        h = x.type(self.dtype)
        c = h[:,:-1,...]
        gra = self.highway_forward2(c)
        anch, cal = self.highway_forward1(c)
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:
                emb = emb.squeeze()
            if ind == 0:
